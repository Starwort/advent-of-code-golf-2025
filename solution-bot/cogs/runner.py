...
from aoc_helper.data import DEFAULT_YEAR as YEAR
from aoc_helper.interface import days_in_year
from bot import Bot
...
class Runner(commands.Cog):
    ...
    async def submit(
        ...
        *,
        ...
        """
        if day < 1 or day > days_in_year(YEAR):
            await ctx.reply(f"Invalid day - must be between 1 and {days_in_year(YEAR)}")
            return
